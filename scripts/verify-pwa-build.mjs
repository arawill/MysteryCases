...
if (!existsSync(dist)) {
  ...
  const faviconPath = join(dist, 'favicon.ico')
  const appleTouchIconPath = join(dist, 'apple-touch-icon-180x180.png')
  if (!existsSync(serviceWorkerPath)) fail('sw.js is missing.')
  if (!existsSync(manifestPath)) fail('manifest.webmanifest is missing.')
  if (!existsSync(faviconPath)) fail('favicon.ico is missing.')
  if (!existsSync(appleTouchIconPath)) fail('apple-touch-icon-180x180.png is missing.')
