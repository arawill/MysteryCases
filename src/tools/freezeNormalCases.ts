...
const rejects: Record<string, number> = { wrongPack: 0, generationFailure: 0, boardDuplicate: 0, nearDuplicate: 0, rosterTooSimilar: 0, avatarTooSimilar: 0, repeatedVictim: 0, repeatedKiller: 0, solutionDuplicate: 0, clueDuplicate: 0 }
...
const data: FrozenNormalCaseSet = { formatVersion: 1, proceduralGenerationVersion: PROCEDURAL_GENERATION_VERSION, cases: allFrozen.filter(item => !(item.difficulty === 1 && item.caseNumber === 1)) }
...
const report = { freezeFormatVersion: 1, proceduralGenerationVersion: PROCEDURAL_GENERATION_VERSION, totalCases: 400, generatedCases: 399, manualCases: 1, candidatesEvaluated, rejects, difficulties: Object.fromEntries([...perDifficulty.entries()].map(([difficulty, cases]) => [difficulty, { count: cases.length, packs: Object.fromEntries(PACK_IDS.map(pack => [pack, cases.filter(item => item.scenarioPackId === pack).length])), candidateAttempts: cases.map(item => item.generation.candidateAttempt), seedOffsets: cases.map(item => item.generation.seedOffset) }])), globalPacks: Object.fromEntries(PACK_IDS.map(pack => [pack, allFrozen.filter(item => item.scenarioPackId === pack).length])), fingerprints: [...fullFingerprints], duplicateCount: 0, nearDuplicateRejects: rejects.nearDuplicate, frozenSourceBytes: Buffer.byteLength(source(data)) }
