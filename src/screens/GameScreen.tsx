...
function GameSession({ gameCase, eyebrowLabel, onCompletionAcknowledged, onCaseCompleted, recordGlobalCompletion = true, completionId }: GameScreenProps) {
  ...
  const quickSelector = <>{globalEvidence}<div className="mobile-character-strip" aria-label="Selector rápido de personas"><p className="eyebrow">PERSONA ACTIVA</p><div>{gameCase.characters.map(character => <button key={character.id} className={selectedId === character.id ? 'active' : ''} onClick={() => selectCharacter(character)} aria-pressed={selectedId === character.id}><span><CharacterAvatar character={character} /></span><small>{character.name}</small>{character.isVictim && <i>V</i>}{placements.some(item => item.characterId === character.id) && <b>✓</b>}</button>)}</div></div></>
