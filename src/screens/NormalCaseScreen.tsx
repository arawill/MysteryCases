...
function LoadedNormalCase({ difficulty, caseNumber }: { difficulty: DifficultyRating; caseNumber: number }) { const generated = useMemo(() => getCachedNormalCase({ difficulty, caseNumber }), [difficulty, caseNumber]); const [notice, setNotice] = useState(''); const complete = (assists: { review: number; exclusion: number; positionChecks: number }) => { const before = loadNormalProgress(); const wasNextLocked = difficulty < 5 && !isDifficultyUnlocked((difficulty + 1) as DifficultyRating, before); const next = markNormalCaseCompleted(difficulty, caseNumber); recordInvestigationCompletion({ mode: 'normal', logicalId: `normal-d${difficulty}-c${String(caseNumber).padStart(2, '0')}`, difficulty, assists }); clearCaseSave(generated.caseData.id); if (wasNextLocked && difficulty < 5 && isDifficultyUnlocked((difficulty + 1) as DifficultyRating, next)) setNotice(`Has desbloqueado la dificultad ${formatDifficultyStars((difficulty + 1) as DifficultyRating)}.`); announceAchievements(reconcileCurrentAchievements().newlyUnlocked) }; return <div className="case-route"><AppHeader back/><div className="normal-case-notice">{notice && <p className="unlock-banner" role="status">{notice}</p>}<GameScreen gameCase={generated.caseData} eyebrowLabel={`CASOS NORMALES · ${formatDifficultyStars(difficulty)} · EXPEDIENTE ${String(caseNumber).padStart(2, '0')}`} onCaseCompleted={complete} recordGlobalCompletion={false}/></div></div> }
