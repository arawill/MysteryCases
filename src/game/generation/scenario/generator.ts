...
import { generateScenarioEdgeFeatures } from './edgeFeatures'
import { assignScenarioTraits } from './traits'
...
const templateFrom = (profile: ScenarioProfile, board: GenerationTemplate['board'], seed: number): GenerationTemplate | undefined => { const edgeFeatures = generateScenarioEdgeFeatures(board, profile.rows, profile.columns, profile.difficulty, seed); if (profile.difficulty >= 4 && !edgeFeatures) return undefined; const traits = assignScenarioTraits(profile.characters, profile.difficulty, seed); if (!traits) return undefined; return { id: `${profile.id}-scenario-${seed}`, title: profile.title, intro: profile.intro, difficulty: profile.difficulty, rows: profile.rows, columns: profile.columns, zones: profile.zones.map(zone => ({ ...zone })), board: board.map(cell => ({ ...cell, ...(cell.object ? { object: { ...cell.object } } : {}) })), characters: traits.characters, ...(edgeFeatures ? { edgeFeatures: edgeFeatures.map(feature => ({ ...feature, segments: feature.segments.map(segment => ({ position: { ...segment.position }, side: segment.side })) })) } : {}), ...(traits.definitions ? { traitDefinitions: traits.definitions.map(definition => ({ ...definition })) } : {}) } }
