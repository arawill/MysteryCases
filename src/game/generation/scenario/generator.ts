...
export function generateScenarioTemplate(profile: ScenarioProfile, options: GenerateScenarioOptions): GeneratedScenario {
  ...
  const random = createSeededRandom(options.seed); const stats: ScenarioStats = { scenarioAttempts: 0, zoneLayoutAttempts: 0, objectPlacementAttempts: 0, feasibilityChecks: 0, objectsPlaced: 0, occupiableCells: 0, blockedCells: 0 }
  for (let attempt = 1; attempt <= maxScenarioAttempts; attempt += 1) {
    stats.scenarioAttempts = attempt; stats.zoneLayoutAttempts += 1; const layout = generateZoneLayout(profile, random, minZoneCells); if (!layout) continue
    const selectedObjects = shuffle(profile.objects, random).slice(0, objectCount); const objectResult = placeScenarioObjects(layout, selectedObjects, random, minOccupiableCellsPerZone); if (!objectResult) continue
    stats.objectPlacementAttempts += objectResult.attempts; const template = templateFrom(profile, objectResult.board, options.seed); const errors = validateGenerationTemplate(template); if (errors.length > 0) continue
    stats.feasibilityChecks += 1
    try { generateValidPlacement(template, random, 10000) } catch { continue }
