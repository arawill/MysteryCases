...
export function createScenarioProfile(caseData: GameCase): ScenarioProfile {
  ...
  return { id: caseData.id, title: caseData.title, intro: caseData.intro, difficulty: caseData.difficulty, rows: caseData.rows, columns: caseData.columns, characters: caseData.characters.map(character => ({ id: character.id, name: character.name, avatar: character.avatar, avatarImage: character.avatarImage, gender: character.gender, roleId: character.roleId, roleLabel: character.roleLabel, isVictim: character.isVictim })), zones: caseData.zones.map(({ labelAnchor: _labelAnchor, labelEdgeAnchor: _labelEdgeAnchor, ...zone }) => ({ ...zone })), objects: [...objects.values()].map(object => ({ ...object! })) }
}
