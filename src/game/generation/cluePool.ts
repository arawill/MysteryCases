...
export function buildTrueCluePool(template: GenerationTemplate, solution: Placement[]): CandidateClue[] {
  const pool: CandidateClue[] = []; const caseData = clueCase(template, solution); const ids = objectIds(template)
  for (const subject of template.characters) {
    ...
    for (const objectId of ids) {
      ...
      if (cell.object?.id !== objectId) add(pool, subject.id, { id: `gen-${subject.id}-not-on-object-${objectId}`, type: 'notOnObject', text: `No estaba en una casilla con ${objectLabel(template, objectId)}.`, objectId })
      if (!beside) add(pool, subject.id, { id: `gen-${subject.id}-not-beside-object-${objectId}`, type: 'notBesideObject', text: `No estaba junto a ${objectLabel(template, objectId)}.`, objectId })
    }
    for (const zone of template.zones) if (zone.id !== cell.zoneId) add(pool, subject.id, { id: `gen-${subject.id}-not-zone-${zone.id}`, type: 'notZone', text: `No estaba en ${zone.name.toLowerCase()}.`, zoneId: zone.id })
