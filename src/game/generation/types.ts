import type { BoardCell, GameCase, Zone } from '../types'
export interface GenerationCharacter { id: string; name: string; avatar: string; isVictim: boolean }
export interface GenerationTemplate { id: string; title: string; intro: string; difficulty: string; rows: number; columns: number; zones: Zone[]; board: BoardCell[]; characters: GenerationCharacter[] }
