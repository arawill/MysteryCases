...
import { clueFamily, getMinimumCluesPerCharacter, isPersonRelation, isPositiveAnchor } from './clueSemantics'
...
export function generatePuzzle(template: GenerationTemplate, options: GeneratePuzzleOptions): GeneratedPuzzle {
  validateSeed(options.seed); const maxPlacementAttempts = options.maxPlacementAttempts ?? 10000; const minCluesPerCharacter = options.minCluesPerCharacter ?? getMinimumCluesPerCharacter(template.difficulty); const minimizeClues = options.minimizeClues ?? true; validatePositiveInteger(maxPlacementAttempts, 'maxPlacementAttempts'); validateNonNegativeInteger(minCluesPerCharacter, 'minCluesPerCharacter')
  const random = createSeededRandom(options.seed); const placement = generateValidPlacement(template, random, maxPlacementAttempts); const pool = shuffle(buildTrueCluePool(template, placement.solution).filter(candidate => !options.procedural || allowsClue(template.difficulty, candidate.clue)), random); const globalPool = shuffle(buildTrueGlobalCluePool(template, placement.solution), random); const stats: GenerationStats = { placementAttempts: placement.attempts, candidateClues: pool.length + globalPool.length, selectedClues: 0, removedClues: 0, solverCalls: 0 }
  const selected: CandidateConstraint[] = [], requirements = difficultyRequirements(template.difficulty)
  for (const character of template.characters) { const available = pool.filter(candidate => candidate.characterId === character.id).sort((a, b) => cluePriority(a) - cluePriority(b)); const chosen: CandidateCharacterClue[] = []; const take = (predicate: (candidate: CandidateCharacterClue) => boolean) => { const candidate = available.find(item => !chosen.some(selected => selected.clue.id === item.clue.id) && predicate(item) && canAddReadableClue([...selected.filter((item): item is CandidateCharacterClue => item.kind === 'character'), ...chosen], item)); if (candidate) chosen.push(candidate) }; take(candidate => isPositiveAnchor(candidate.clue)); take(candidate => clueFamily(candidate.clue) !== clueFamily(chosen[0]?.clue ?? candidate.clue)); while (chosen.length < minCluesPerCharacter) take(() => true); if (chosen.length < minCluesPerCharacter) throw new Error(`Not enough readable candidate clues for ${character.id}.`); selected.push(...chosen) }
  ...
  selectRequired(candidate => isPersonRelation(candidate.clue), 1)
  ...
  while (result.solutionsFound > 1) {
    ...
    const ranked = remaining.filter(candidate => !selected.some(chosen => chosen.clue.id === candidate.clue.id) && (candidate.kind === 'global' ? selected.filter(item => item.kind === 'global').length < requirements.maxGlobals : true)).map((candidate, index) => ({ candidate, index, eliminated: result.solutions.filter(solution => evaluateCandidateConstraint(candidate, current, solution) === 'violated').length })).filter(item => item.eliminated > 0).sort((a, b) => b.eliminated - a.eliminated || a.index - b.index)
