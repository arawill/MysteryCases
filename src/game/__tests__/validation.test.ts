...
describe('validateCaseDefinition', () => {
  ...
  it.each([{ row: 1, column: 2 }, { row: 1, column: 3 }])('detecta incoherencia occupiable en la celda $row:$column', ({ row, column }) => { const invalid = clone(); const cell = invalid.board.find(candidate => candidate.row === row && candidate.column === column); if (!cell) throw new Error('Celda de prueba no encontrada.'); cell.occupiable = !cell.occupiable; const errors = validateCaseDefinition(invalid).join(' '); expect(errors).toContain(`Incoherencia occupiable en celda ${row}:${column}`) })
})
