...
describe('5.5E.1 procedural vocabulary', () => {
  it('versions procedural save IDs without changing logical normal IDs', () => {
    expect(PROCEDURAL_GENERATION_VERSION).toBe(7)
    expect(getVersionedProceduralCaseId('normal-d2-c01')).toBe('normal-d2-c01-g7')
  ...
  it('keeps manual Normal IDs while versioning procedural Normal, Daily and Infinite save slots', () => {
    ...
    expect(generateNormalCase({ difficulty: 1, caseNumber: 4 }).caseData.id).toBe(case004.id)
    expect(generateNormalCase({ difficulty: 1, caseNumber: 16 }).caseData.id).toBe(getVersionedProceduralCaseId(getNormalCaseId(1, 16)))
    ...
    expect(getDailyPuzzleId(date, 1)).not.toContain('-g7')
    expect(getInfiniteCaseId(1, 42)).not.toContain('-g7')
  })
