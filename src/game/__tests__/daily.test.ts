...
import { getDailyCaseId, getDailyDateKey, getDailyDifficultySeed, getDailySeed } from '../daily/date'
...
const rosterForDate = (date: Date) => { const seed = getDailyDifficultySeed(date, 1); return buildCharacterRoster({ difficulty: 1, seed, scenarioPack: selectScenarioPack(seed) }) }
const rosterSignature = (date: Date) => rosterForDate(date).map(person => `${person.name}:${person.avatarImage}:${person.roleId}`).join('|')
const clueQuality = (generated: ReturnType<typeof generateDailyCase>) => {
  ...
  expect(generated.caseData.characters.every(character => character.isVictim ? character.clues.length === 0 : character.clues.length >= 2 && character.clues.some(clue => !isNegativeClue(clue)) && !(character.clues.some(clue => clue.type === 'row') && character.clues.some(clue => clue.type === 'column')))).toBe(true)
...
describe('daily case', () => {
  ...
  it('does not mutate Case001 or procedural catalogs', () => { const originalCase = structuredClone(case001), originalNames = structuredClone(nameCatalog), originalAvatars = structuredClone(avatarCatalog), originalPacks = structuredClone(scenarioPacks); generateDailyCase(dateA); expect(case001).toEqual(originalCase); expect(nameCatalog).toEqual(originalNames); expect(avatarCatalog).toEqual(originalAvatars); expect(scenarioPacks).toEqual(originalPacks) })
})
