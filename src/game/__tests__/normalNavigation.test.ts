...
const progress = (highestUnlocked: DifficultyRating): NormalModeProgress => {
  ...
  for (let difficulty = 1; difficulty < highestUnlocked; difficulty += 1) completed[difficulty as DifficultyRating] = Array.from({ length: 40 }, (_, index) => index + 1)
...
describe('normal case navigation', () => {
  ...
  it('does not cross into a blocked difficulty', () => {
    expect(getNormalCaseNavigation({ difficulty: 1, caseNumber: 80, progress: progress(1) }).next).toBeNull()
  })
  it('crosses unlocked difficulties in both directions', () => {
    expect(getNormalCaseNavigation({ difficulty: 1, caseNumber: 80, progress: progress(2) }).next).toEqual({ difficulty: 2, caseNumber: 1 })
    expect(getNormalCaseNavigation({ difficulty: 2, caseNumber: 1, progress: progress(2) }).previous).toEqual({ difficulty: 1, caseNumber: 80 })
    expect(getNormalCaseNavigation({ difficulty: 4, caseNumber: 80, progress: progress(5) }).next).toEqual({ difficulty: 5, caseNumber: 1 })
  })
  it('never wraps beyond the final case', () => {
    expect(getNormalCaseNavigation({ difficulty: 5, caseNumber: 80, progress: progress(5) }).next).toBeNull()
  })
})
