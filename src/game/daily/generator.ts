...
export function generateDailyCase(date: Date): GeneratedDailyCase { const dateKey = getDailyDateKey(date), baseSeed = getDailySeed(date), characters = selectDailyCharactersForDate(date); const base = createScenarioProfile(case001); const profile = { ...base, id: getDailyCaseId(date), title: 'Caso diario', intro: 'Un nuevo expediente espera hoy. Reconstruye dónde estaba cada persona y descubre quién se quedó a solas con la víctima.', difficulty: 1 as const, characters }
  for (let offset = 0; offset < 100; offset += 1) { const effectiveSeed = (baseSeed + offset) >>> 0; try { const scenario = generateScenarioTemplate(profile, { seed: effectiveSeed }); const puzzle = generatePuzzle(scenario.template, { seed: effectiveSeed, minCluesPerCharacter: 2 }); const caseData = { ...puzzle.caseData, id: getDailyCaseId(date) }; const errors = validateCaseDefinition(caseData); const analysis = analyzeCase(caseData); const killer = findKiller(caseData, caseData.solution); if (!hasReadableClues(caseData) || errors.length || analysis.status !== 'unique' || analysis.matchesCanonical !== true || solveCase(caseData).solutionsFound !== 1 || !killer) continue; return { caseData, dateKey, baseSeed, effectiveSeed, seedOffset: offset, killerId: killer.id, scenarioAttempts: scenario.stats.scenarioAttempts } } catch { continue } }
