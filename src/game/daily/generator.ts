...
export function generateDailyCase(date: Date): GeneratedDailyCase {
  const dateKey = getDailyDateKey(date), baseSeed = getDailySeed(date), characters = selectDailyCharactersForDate(date)
  const base = createScenarioProfile(case001)
  const profile = { ...base, id: getDailyCaseId(date), title: 'Caso diario', intro: 'Un nuevo expediente espera hoy. Reconstruye dónde estaba cada persona y descubre quién se quedó a solas con la víctima.', difficulty: 1 as const, characters }
  for (let offset = 0; offset < 100; offset += 1) {
    ...
    try {
      ...
      const caseData = { ...puzzle.caseData, id: getDailyCaseId(date) }
...
export function getCachedDailyCase(date: Date): GeneratedDailyCase { const key = getDailyDateKey(date), cached = dailyCaseCache.get(key); if (cached) return cached; const generated = generateDailyCase(date); dailyCaseCache.set(key, generated); return generated }
