...
import { selectDailyCharactersWithPrevious } from './characters'
...
export function generateDailyCase(date: Date): GeneratedDailyCase { const dateKey = getDailyDateKey(date), baseSeed = getDailySeed(date), previous = new Date(date.getFullYear(), date.getMonth(), date.getDate() - 1), characters = selectDailyCharactersWithPrevious(baseSeed, getDailySeed(previous)); const base = createScenarioProfile(case001); const profile = { ...base, id: getDailyCaseId(date), title: 'Caso diario', intro: 'Un nuevo expediente espera hoy. Reconstruye dónde estaba cada persona y descubre quién se quedó a solas con la víctima.', difficulty: 1 as const, characters }
