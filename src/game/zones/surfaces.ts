...
export const isZoneSurface = (value: unknown): value is ZoneSurface => surfaces.some(surface => surface === value)
...
export function resolveZoneSurface(zone?: Pick<Zone, 'surface' | 'tone'>): ZoneSurface {
  if (zone?.surface !== undefined) return isZoneSurface(zone.surface) ? zone.surface : 'generic'
