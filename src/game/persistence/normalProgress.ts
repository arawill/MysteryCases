...
import { isActiveNormalCaseNumber, NORMAL_CASE_COUNT, NORMAL_UNLOCK_CASE_COUNT } from '../normal/constants'
...
const normalise = (value: unknown): NormalModeProgress => {
  ...
  for (const difficulty of difficulties) {
    ...
    completed[difficulty] = Array.isArray(values) ? [...new Set(values.filter(isActiveNormalCaseNumber))].sort((a, b) => a - b) : []
  }
...
export function countCompletedUnlockCases(difficulty: DifficultyRating, progress: NormalModeProgress) { return progress.completedCaseNumbersByDifficulty[difficulty].filter(isActiveNormalCaseNumber).length }
export function isDifficultyUnlocked(difficulty: DifficultyRating, progress: NormalModeProgress): boolean { if (difficulty === 1) return true; const previous = (difficulty - 1) as DifficultyRating; return isDifficultyUnlocked(previous, progress) && countCompletedUnlockCases(previous, progress) === NORMAL_UNLOCK_CASE_COUNT }
...
export function isNormalCaseCompleted(difficulty: DifficultyRating, caseNumber: number, progress: NormalModeProgress) { return isActiveNormalCaseNumber(caseNumber) && progress.completedCaseNumbersByDifficulty[difficulty].includes(caseNumber) }
export function markNormalCaseCompleted(difficulty: DifficultyRating, caseNumber: number, storage: Storage = localStorage) { const progress = loadNormalProgress(storage); if (!isActiveNormalCaseNumber(caseNumber) || !isDifficultyUnlocked(difficulty, progress)) return progress; const next: NormalModeProgress = { ...progress, completedCaseNumbersByDifficulty: { ...progress.completedCaseNumbersByDifficulty, [difficulty]: [...new Set([...progress.completedCaseNumbersByDifficulty[difficulty], caseNumber])].sort((a, b) => a - b) } }; return saveNormalProgress(next, storage) }
