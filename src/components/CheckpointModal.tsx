...
import { CHECKPOINT_DESCRIPTION_LIMIT, CHECKPOINT_NAME_LIMIT, type CaseCheckpoint } from '../game/checkpoints'
...
export function CheckpointModal({ checkpoints, onCreate, onRestore, onDelete, onClose }: CheckpointModalProps) {
  ...
  const confirm = () => {
    if (!pending) return
    if (pending.action === 'restore') { onRestore(pending.id); setFeedback('Hipótesis restaurada. Los usos de ayudas se conservan.') }
    else { onDelete(pending.id); setFeedback('Punto de guardado eliminado.') }
    setPending(null)
    nameInput.current?.focus()
  }
  ...
  return <dialog ref={dialog} className="checkpoint-dialog" aria-labelledby="checkpoint-title" aria-describedby="checkpoint-description" onCancel={event => { event.preventDefault(); onClose() }}>
    ...
    <form className="checkpoint-form" onSubmit={create}>
      ...
      <button className="primary" type="submit" disabled={!name.trim()}>CREAR PUNTO DE GUARDADO</button>
    </form>
